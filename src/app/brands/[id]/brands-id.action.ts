...
export async function getBrandDetailsAction(id: string): Promise<BrandsData> {
  const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL || 'https://ecommerce.routemisr.com'}/api/v1/brands/${id}`, {
...
export async function getProductsByBrandAction(id: string): Promise<AllproductsData[]> {
  const response = await fetch(`${process.env.NEXT_PUBLIC_BASE_URL || 'https://ecommerce.routemisr.com'}/api/v1/products?brand=${id}`, {
    next: { revalidate: 3600 },
  });
